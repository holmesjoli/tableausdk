        ...
        for package, src_dir, build_dir, filenames in self.data_files:
            for filename in filenames:
                ...
                if os.path.islink(src_file):
                    ...
                    # Use dir_util.copy_tree instead of shutil.copytree
                    # because dst_file might be a directory that already exists
                    from distutils.dir_util import copy_tree
                    copy_tree(src_file, dst_file, preserve_symlinks=True)
                else:
...
if sys.platform == 'win32':
   copy_list = ['bin/*', 'bin/hyper/*', 'lib/*.lib']
elif sys.platform.startswith('linux'):
   copy_list = ['bin/*', 'bin/hyper/*', 'lib/*so*']
